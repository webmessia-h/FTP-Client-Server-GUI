...
Server::Server(QObject *parent)
    : QObject(parent), m_fileListModel(new FileListModel(this)),
      m_connected(false), m_launched(false),
...
void Server::launch(const QString Qipaddr, QString Qport) {
  ...
  threadPool->enqueue([this]() {
    if (!Network::create_server(server_sockfd, server_addr, this->ip,
                                this->port) ||
        !Network::bind_to_port(this->port, this->server_sockfd,
                               this->server_addr)) {
      QMetaObject::invokeMethod(this, "setLaunched", Qt::QueuedConnection,
                                Q_ARG(bool, false));
      return;
    }
    QMetaObject::invokeMethod(this, "setLaunched", Qt::QueuedConnection,
...
void Server::accept() {
  threadPool->enqueue([this]() {
    if (!Network::listen_client(server_sockfd) ||
        !Network::accept_connection(server_sockfd,
                                    this->communication_sockfd)) {
      return;
    }
    QMetaObject::invokeMethod(this, "setIsConnected", Qt::QueuedConnection,
                              Q_ARG(bool, true));
  });
  return;
}
...
bool Server::receive_file_list() {
  ...
  while (true) {
    int bytes_received =
        Network::receive_data(communication_sockfd, buffer, bufferSize);
    if (bytes_received < 0) {
  ...
  // Update the model
  if (m_fileListModel) {
    bool success = QMetaObject::invokeMethod(m_fileListModel, "setStringList",
                                             Qt::QueuedConnection,
...
bool Server::receive_file(const QString &Qfilename) {
  ...
  // Get size of file
  size_t file_size;
  if (Network::receive_data(transfer_sockfd, &file_size, sizeof(file_size)) ==
      -1) {
    std::cerr << "Error receiving file size" << std::endl;
    return false;
  }
  // Set default chunk size of 1Kb
  size_t chunk_size = 1024;
  char *buffer = new char[chunk_size];
  // Proccess the desired filename from UI
  ...
  try {
    ...
    while (total_bytes_received < file_size) {
      ...
      if (Network::send_data(this->communication_sockfd, ACK, sizeof(ACK)) ==
          -1) {
      ...
      chunk_size = remaining_bytes > 1024 ? 1024 : remaining_bytes;

      // Receive chunk of file
      ssize_t bytes_received =
          Network::receive_data(this->transfer_sockfd, buffer, chunk_size);
      if (bytes_received == -1) {
      ...
      // Write chunk of a file
      if (!Platform::write_file(filename, buffer, bytes_received, file)) {
        std::cerr << "Error writing chunk to file." << std::endl;
        Platform::cleanup_handler(buffer);
        file.close();
        throw std::runtime_error("Write chunk error");
      }
      ...
      if (new_percentage != percentage) {
        ...
        if ((percentage - 1) % 10 == 0) {
          if (Network::send_data(communication_sockfd, CHPORT,
                                 sizeof(CHPORT)) == -1) {
...
QString Server::getIp() const { return QString(this->ip); }

bool Server::isLaunched() const { return this->m_launched; }
...
bool Server::isConnected() const { return m_connected; }
...
QStringList Server::getFileList() const { return m_fileList; }
FileListModel *Server::fileListModel() const { return m_fileListModel; }

float Server::progress() const { return m_progress; }
...
bool Server::success() const { return m_transferSuccess; }
